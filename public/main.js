...
function main(){
  let cuadradosUsables = [];
  const cuadradosTablero = document.getElementsByClassName("Cuadrado");
  const piezas = document.getElementsByClassName("pieza");
  const imagenesPiezas=document.getElementsByTagName("img");
  
  establecerCuadradosTablero();
  configurarPiezas();

  function establecerCuadradosTablero(){
    for(let i=0; i< cuadradosTablero.length; i++){
      cuadradosTablero[i].addEventListener("dragover",allowDrop);
      cuadradosTablero[i].addEventListener("drop",drop);
      let fila=8-Math.floor(i/8);
      let columna = String.fromCharCode(97+(i%8));
      let cuadrado = cuadradosTablero[i];
      cuadrado.id = columna + fila;
    }
  }
  
  function configurarPiezas() {
    for(let i=0; i< piezas.length; i++){
      piezas[i].addEventListener("dragstart",drag);
      piezas[i].setAttribute("draggable",true);
      piezas[i].id=piezas[i].className.split(" ")[1]+piezas[i].parentElement.id;
    } 
    for(let i=0; i< imagenesPiezas.length; i++){
      imagenesPiezas[i].setAttribute("draggable",false);
    } 
  }
  ...
  function drag(ev){
    const pieza=ev.target;
    ev.dataTransfer.setData("text",pieza.id);
  }
  function drop(ev){
    ev.parentDefault();
    let data=ev.dataTransfer.getData("text");
    const pieza = document.getElementsById(data);
    const cuadradoDestino=ev.currentTarget;
    let cuadradoDestinoId=cuadradoDestino.id;
    cuadradoDestino.appendChild(pieza);
  }
...
// const boardSquares = document.getElementsByClassName("Cuadrado");
// const pieces = document.getElementsByClassName("pieza");
...
//     let pieceType = "";
//     let pieceId="";
//     if (square.querySelector(".pieza")) {
//       color = square.querySelector(".pieza").getAttribute("color");
//       pieceType = square.querySelector(".pieza").classList[1];
//       pieceId=square.querySelector(".pieza").id;
//     } else {
//       color = "blank";
//       pieceType = "blank";
//       pieceId ="blank";
...
//   if (
//     (isWhiteTurn && pieceColor == "Blanco") ||
//     (!isWhiteTurn && pieceColor == "Negro")
//   ) {
...
// function drop(ev) {
//   ev.preventDefault();
//   let data = ev.dataTransfer.getData("texto");
